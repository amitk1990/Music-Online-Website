...
drop procedure create_or_edit_bands;

DELIMITER $$
create procedure create_or_edit_bands(
in User varchar(20),
in B varchar(20),
inout update_user_flag int)
begin 
declare count int default 0;
select count(*) into count from user_band u where u.userid = User and u.bid=B;
if update_user_flag=0 and count<1 then
		set update_user_flag=2;
		insert into user_band values(User,B,now());
elseif update_user_flag=1 then 
		set update_user_flag=3;
		delete from user_band where userid=User and bid=B;
end if;
end $$
DELIMITER ;

insert into user_band values("user1","band2",now());

set @flag=0;
call create_or_edit_bands("user1","band3",@flag);
select @flag;
